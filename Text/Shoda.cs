...
namespace Daliboris.Text {
	public class Shoda : IShoda {
		...
		public string Nasledujici {
			get {
				return mstrNasledujici;
			}
			set {
				mstrNasledujici = value;
			}
		}
		...
		public Shoda() { }
		public Shoda(string strPredchozi, string strAktualni, string strNasledujici, bool blnRetrograd, StringComparison scpZpusobPorovnani)  {
		...
		public Shoda(string strPredchozi, string strAktualni, string strNasledujici, StringComparison scpZpusobPorovnani)
		...
		private void UrciShodu() {
			...
			int iNasledujici = Shoda.PocetShodnychZnaku(mstrAktualni, mstrNasledujici, mblnRetrograd, mscpZpusobPorovnani);
			if (!mblnRetrograd) {
		...
		public static IList<Shoda> UrciShodu(string strSeznamSlov, string strOddelovac, bool blnRetrograd, StringComparison scpZpusobPorovnani) {
			...
			for (int i = 0; i < iDelka; i++) {
				if (i == 0) {
					...
					lstShoda.Add(new Shoda(asSeznam[i - 1], asSeznam[i], asSeznam[i + 1], blnRetrograd, scpZpusobPorovnani));
				}
				else {
		...
		public static IList<Shoda> UrciShodu(string strSeznamSlov, string strOddelovac, StringComparison scpZpusobPorovnani) {
