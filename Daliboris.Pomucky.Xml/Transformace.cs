...
namespace Daliboris.Pomucky.Xml
{
	public static partial class  Transformace
	{
			...
			while (reader.Read())
			{
				switch (reader.NodeType)
				{
					...
					case XmlNodeType.XmlDeclaration:
                        ...
                        writer.WriteComment("InsertSpacesAfterTagsWithPunctuation");
						break;
					default:
						SerializeNode(reader, writer);
						break;
	    ...
	    public static void OznacitSamosatnouMezeruJakoElement(string vstup, string vystup, XmlReaderSettings xmlReaderSettings, string elementName, List<string> kdeMezeraNemuzeByt)
	    {
	        Console.WriteLine("{0} => {1}, Označit samostatnou mezeru jako element", vstup, vystup);
	        using (XmlReader xr = XmlReader.Create(vstup, xmlReaderSettings))
	        {
	            using (XmlWriter xw = XmlWriter.Create(vystup))
	            {
	                OznacitSamosatnouMezeruJakoElement(xr, xw, elementName, kdeMezeraNemuzeByt);
	            }
	        }
        }

	    private static void OznacitSamosatnouMezeruJakoElement(XmlReader reader, XmlWriter writer, string elementName, List<string> kdeMezeraNemuzeByt)
	    {
	        string currentParentElement = null;
            Dictionary<int, string> currentElements = new Dictionary<int, string>();
	        ...
	        while (reader.Read())
	        {
	            ...
	            switch (reader.NodeType)
	            {
	                case XmlNodeType.XmlDeclaration:
	                    SerializeNode(reader, writer);
	                   // writer.WriteComment("OznacitSamosatnouMezeruJakoElement");
                        writer.WriteComment("MarkSingleSpaceAsElement");
	                    break;
                    case XmlNodeType.Whitespace:
                        if(currentElements.ContainsKey(level - 1))
                            currentParentElement = currentElements[level - 1];
                        else
                        {
                            if(level == 0) //komentář v před začátkem kořenového elementu
                                break;
                            currentParentElement = currentElements[level];
                        }

                        if (reader.Value == " " && kdeMezeraNemuzeByt != null && !kdeMezeraNemuzeByt.Contains(currentParentElement))
	                    {
                    ...
                    case XmlNodeType.Element:
                        ...
                        if (!currentElements.ContainsKey(level))
                            currentElements.Add(level, element);
                        else
                        {
                            currentElements[level] = element;
                        }
                        goto default;
                    case XmlNodeType.EndElement:
                        currentElements.Remove(level);
                        goto default;
	                default:
	                    SerializeNode(reader, writer);
	                    break;
	...
	public class XmlElementInfo : ICloneable
	{
		...
		/// </summary>
		public XmlElementInfo(string name, int depth, bool isElementEmpty) : this()
		{
			Name = name;
			Depth = depth;
	    ...
	    public static Dictionary<string, string> ReadAttributes(XmlReader reader)
	    {
            
	        Dictionary<string, string> attributes = new Dictionary<string, string>();
	        if (!reader.HasAttributes) return attributes;

            attributes = new Dictionary<string, string>(reader.AttributeCount);
