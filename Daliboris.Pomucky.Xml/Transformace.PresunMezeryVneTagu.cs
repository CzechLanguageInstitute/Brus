...
namespace Daliboris.Pomucky.Xml
{
	public static partial class Transformace
	{
		...
		public static void PresunoutMezeryVneTagu2(XmlReader reader,
			...
			string znakyInterpunkce)
		{
			PresunoutMezeryVneTagu2(reader, writer, tagyPredNimizSeMezeraNepresouva, tagyKdeSeMezeraPresouvaZaZnacku,
                tagyKdeSePresouvaniVynechava, new List<string>(), znakyInterpunkce, null);
		}
		...
		/// <param name="znakyInterpunkce">Seznam znaku, před nimiž se nevkládá mezera.</param>
		public static void PresunoutMezeryVneTagu2(XmlReader reader, XmlWriter writer, List<string> tagyPredNimizSeMezeraNepresouva, 
            List<string> tagyKdeSeMezeraPresouvaZaZnacku, List<string> tagyKdeSePresouvaniVynechava, 
            List<string> tagyKdeSePresouvaniIgnoruje, string znakyInterpunkce,
		    List<string> tagyPresKtereSeMezeryNepresouvaji
            )
		{
            ...
            if(tagyPresKtereSeMezeryNepresouvaji == null)
                tagyPresKtereSeMezeryNepresouvaji = new List<string>();
			...
			Zacatek:
				...
				switch (reader.NodeType)
				{
					case XmlNodeType.Element:
					    ...
					    currentXmlElementInfo.Attributes = XmlElementInfo.ReadAttributes(reader);
					...
					case XmlNodeType.Text:
						{
						    ...
						    if (reader.NodeType == XmlNodeType.SignificantWhitespace &&  actualSpace.Text == actualSpace.TextWithoutTrailingSpace &&
						        actualSpace.Text.Replace("\r", "").Replace("\n", "").Replace("\t", "").Length == 0)
						        previousSpace = null;
						    if (previousSpace != null && !previousSpace.EndsWithSpace && actualSpace.EndsWithSpace)
						        previousSpace = null;

							if (!tagyKdeSeMezeraPresouvaZaZnacku.Contains(currentElementInfo.Name) 
							    || 
							    (previousSpace != null 
							     && currentElementInfo.Name == previousSpace.CurrentElementInfo.Name 
							     && currentElementInfo.Depth == previousSpace.CurrentElementInfo.Depth) 
                                ||
                                (previousSpace != null 
                                 && tagyKdeSeMezeraPresouvaZaZnacku.Contains(currentElementInfo.Name) 
                                 && currentElementInfo.Name == previousSpace.CurrentElementInfo.Name
                                 && currentElementInfo.Depth + 1 == previousSpace.CurrentElementInfo.Depth
							    )
                                ||
							    (previousSpace != null 
							     && tagyKdeSeMezeraPresouvaZaZnacku.Contains(currentElementInfo.Name) 
                                 && currentElementInfo.Name == "note"
                                 && currentElementInfo.Attributes.ContainsKey("type") && currentElementInfo.Attributes["type"] == "print"
							     && currentElementInfo.Attributes.ContainsKey("place") && currentElementInfo.Attributes["place"] == "margin"
							    )
							    )
							{
					...
					*/
						RemoveUnusedKeys(endElementInfo, spacesInDocument, xmlElementInfos, tagyKdeSePresouvaniVynechava, tagyPresKtereSeMezeryNepresouvaji);
					...
					case XmlNodeType.XmlDeclaration:
                        ...
                        writer.WriteComment("MoveSpaceOutsideOfTheElement");
						break;
					default:
						SerializeNode(reader, writer);
						break;
		...
		private static void RemoveUnusedKeys(XmlElementInfo endElementInfo,
            DocumentSpace spacesInDocument, Stack<XmlElementInfo> xmlElementInfos, 
            List<string> tagyKdeSePresouvaniVynechava, List<string> tagyPresKtereSeMezeryNepresouvaji)
		{
		    ...
		    if (tagyPresKtereSeMezeryNepresouvaji.Contains(endElementInfo.Name))
		    {
		        foreach (KeyValuePair<int, Space> kvp in spacesInDocument)
		        {
                    keysToRemove.Add(kvp.Key);
					 ...
					 ))
				{
					...
					else
					{
					    ...
					    if (((!deeperSpace.EndsWithSpace || deeperSpace.SpaceInvalidated) || deeperSpace.CurrentElementInfo.IsElementEmpty) &&
					        ...
					        keysToRemove.Add(deeperSpace.Depth);
                    }
				}
			}
		...
		{
			PresunoutMezeryVneTagu(vstup, vystup, xmlReaderSettings, tagyPredNimizSeMezeraNepresouva, tagyKdeSeMezeraPresouvaZaZnacku, tagyKdeSePresouvaniVynechava, new List<string>(), znakyInterpunkce, null);
		}
	    ...
	    /// <param name="tagyKdeSePresouvaniIgnoruje">Seznam tagů, u nichž se kopíruje celý jejich obsah, aniž se zjišťuje, co je uvnitř. Obvykle se tagy uvedou i mezi <paramref name="tagyPredNimizSeMezeraNepresouva"/> Např. ins, del.</param>
	    /// <param name="znakyInterpunkce">Seznam znaku, před nimiž se nevkládá mezera.</param>
	    /// <param name="tagyPresKtereSeMezeryNepresouvaji"></param>
	    public static void PresunoutMezeryVneTagu(string vstup, string vystup, XmlReaderSettings xmlReaderSettings, 
            List<string> tagyPredNimizSeMezeraNepresouva, List<string> tagyKdeSeMezeraPresouvaZaZnacku, 
            List<string> tagyKdeSePresouvaniVynechava, List<string> tagyKdeSePresouvaniIgnoruje, string znakyInterpunkce,
            List<string> tagyPresKtereSeMezeryNepresouvaji)
		{
			...
			using (XmlReader xr = XmlReader.Create(vstup, xmlReaderSettings))
			{
				using (XmlWriter xw = XmlWriter.Create(vystup))
				{
					...
					PresunoutMezeryVneTagu2(xr, xw, tagyPredNimizSeMezeraNepresouva, tagyKdeSeMezeraPresouvaZaZnacku, 
                        tagyKdeSePresouvaniVynechava, tagyKdeSePresouvaniIgnoruje, znakyInterpunkce, tagyPresKtereSeMezeryNepresouvaji);
