...
namespace Daliboris.Statistiky {
	...
	public class Statistik {
		...
		public static void ExtractFilesForTranscriptorium(string inputFilePath, string outpuDirecotry)
		{
			...
			Sprava jvsSprava = new Sprava();
			using (Package wdPackage = Package.Open(inputFilePath, FileMode.Open, FileAccess.Read))
			{
				PackageRelationship docPackageRelationship = wdPackage.GetRelationshipsByType(Relace.Statistika).FirstOrDefault();
					...
					foreach (PackageRelationship docRelationship in docCollection)
					{
						...
						if (p != null)
						{
							...
							foreach (PackageRelationship pr in pcols)
							{
								...
								string outputFilePath = Path.Combine(outpuDirecotry, String.Format("cs\\{0}\\slova.xml", i));
		...
		public SkupinaJevu NactiStatistiky(string strVstupniSoubor) {
						...
						if (p != null)
						{
							...
							foreach (PackageRelationship pr in pcols)
							{
								...
								else if (prt.ContentType == MediaTypeNames.Text.Xml)
								{
									...
									jevy.Save(path);
									jv = Sprava.NactiZeSouboruXml(path);
								}
								//jvs.Add(jv);
								skj.Add(jv);
							}
		...
		public static void UlozStatistiky(SkupinaJevu skjJevy, string strVystupniSoubor, FormatUlozeniSeznamu fusFormat, bool odstranitTeckuUSlov)
		{
		...
		public static void UlozStatistiky(SkupinaJevu skjJevy, string strVystupniSoubor,
			FormatUlozeniSeznamu fusFormat, bool odstranitTeckuUSlov, List<string> obsahDokumentu, Dictionary<string, int> stylyDokumentu)
		{
			UlozStatistiky(skjJevy, strVystupniSoubor, fusFormat, null, odstranitTeckuUSlov, obsahDokumentu, stylyDokumentu);
		}
		...
		public static void UlozStatistiky(SkupinaJevu skjJevy, string strVystupniSoubor, FormatUlozeniSeznamu fusFormat,
			VlastnostiBalicku vlbVlastnosti, bool odstranitTeckuUSlov, List<string> obsahDokumentu, Dictionary<string, int> stylyDokumentu)
		{
			...
			using (Package package = Package.Open(strVystupniSoubor, FileMode.Create)) {
				...
				Uri partUriStatistiky = PackUriHelper.CreatePartUri(new Uri(AdresarovaStruktura.Statistika + "\\statistika.xml", UriKind.Relative));
				PackagePart pkgpStatistiky = package.CreatePart(partUriStatistiky, MediaTypeNames.Text.Xml, CompressionOption.Maximum);

				foreach (Jevy kvp in skjJevy) {
					if (!gdcSeskupeneJevy.ContainsKey(kvp.Typ)) {
						gdcSeskupeneJevy.Add(kvp.Typ, new SkupinaJevu());
					}
					gdcSeskupeneJevy[kvp.Typ].Add(kvp);
				}
				...
				foreach (KeyValuePair<TypJevu, SkupinaJevu> kvp in gdcSeskupeneJevy) {
					...
					if (kvp.Key == TypJevu.Odstavce) {
						...
						foreach (Jevy jvs in kvp.Value) {
							...
							if (fusFormat == FormatUlozeniSeznamu.Xml) {
								Sprava.UlozDoSouboru(jvs, strSouborJevyTmp, FormatUlozeni.Xml);
							}
							if (fusFormat == FormatUlozeniSeznamu.Text) {
								//TODO Využít CteckuSouboru pro vytvoření textové podoby XML
								Sprava.UlozDoSouboru(jvs, strSouborJevyTmp, FormatUlozeni.Text);
							}

							string strStylId = AdresarovaStruktura.Styl + i.ToString("000");
							string strSlozkaStylu = AdresarovaStruktura.Statistika + "\\" + AdresarovaStruktura.OdstavcoveStyly + "\\" + strStylId;

							Uri partUriObsah = PackUriHelper.CreatePartUri(new Uri(strSlozkaStylu + "\\" + AdresarovaStruktura.Obsah + "\\" + strObsah, UriKind.Relative));
							...
							PackagePart pkgpObsah = package.CreatePart(partUriObsah,
								fusFormat == FormatUlozeniSeznamu.Xml ? MediaTypeNames.Text.Xml : MediaTypeNames.Text.Plain, CompressionOption.Maximum);
							...
							PackageRelationship rel = pkgpStatistiky.CreateRelationship(pkgpStyl.Uri, TargetMode.Internal, Relace.OdstavcovyStyl);
							...
							*/
							...
							rel = UlozitJevyDoBalickuJakoObsah(fusFormat, strPripona, strJevyTmp, AdresarovaStruktura.Statistika + "\\", AdresarovaStruktura.OdstavcoveStyly, strStylId, package, pkgpStyl, jvTrigramy, "trigramy", Relace.Trigramy);
							mtdOdstavcovyStyl.Trigramy.RelaceId = rel.Id;


							rel = UlozitJevyDoBalickuJakoObsah(fusFormat, strPripona, strJevyTmp, AdresarovaStruktura.Statistika + "\\", AdresarovaStruktura.OdstavcoveStyly, strStylId, package, pkgpStyl, jvSlova, "slova", Relace.Slova);
							mtdOdstavcovyStyl.Slova.RelaceId = rel.Id;

							rel = UlozitJevyDoBalickuJakoObsah(fusFormat, strPripona, strJevyTmp, AdresarovaStruktura.Statistika + "\\",
								AdresarovaStruktura.OdstavcoveStyly, strStylId, package, pkgpStyl, jvZnaky, "znaky", Relace.Znaky);
							...
							foreach (KeyValuePair<TypJevu, SkupinaJevu> kvpz in gdcSeskupeneJevy) {
								if (kvpz.Key == TypJevu.Useky) {
									...
									foreach (Jevy jv in kvpz.Value) {
										if (jvs.Identifikator != jv.Identifikator && jv.Identifikator.StartsWith(jvs.Identifikator)) {
											MetadataOJevu mtj = ZiskatMetadataOJevu(++iPoradi, jv);
											mtj.Popis = jv.Popis.Substring(jv.Popis.IndexOf('+') + 1).Trim();
											UlozitZnakovyStyl(jv, package, pkgpStyl, strSlozkaStylu + "\\", fusFormat, iPoradi, strTemp, ref mtj);
					...
					if (kvp.Key == TypJevu.Useky) {
						...
						foreach (Jevy jvs in kvp.Value) {
							...
							UlozitZnakovyStyl(jvs, package, pkgpStatistiky, AdresarovaStruktura.Statistika + "\\", fusFormat, i, strTemp, ref mtj);
				...
				Uri partUriData = PackUriHelper.CreatePartUri(new Uri(AdresarovaStruktura.Statistika + "\\data.xml", UriKind.Relative));
				PackagePart pkgpData = package.CreatePart(partUriData, MediaTypeNames.Text.Xml, CompressionOption.Maximum);
		...
		private static void UlozitStatistikuDoBalickuJakoObsahVsechnyZnaky(FormatUlozeniSeznamu fusFormat, Package package,
	...
	string strPripona, Dictionary<string, int> stylyDokumentu)
		...
		private static void UlozitZnakovyStyl(Jevy jvJevy, Package package, PackagePart pkgpVychoziCastBalicku,
			string strVychoziSlozka, FormatUlozeniSeznamu fusFormat, int intPoradi, string strDocasnaSlozka, ref MetadataOJevu mtdMetadata) {
			...
			if (fusFormat == FormatUlozeniSeznamu.Xml) {
				Sprava.UlozDoSouboru(jvJevy, strSouborJevyTmp, FormatUlozeni.Xml);
			}
			if (fusFormat == FormatUlozeniSeznamu.Text) {
				//TODO Využít CteckuSouboru pro vytvoření textové podoby XML
				Sprava.UlozDoSouboru(jvJevy, strSouborJevyTmp, FormatUlozeni.Text);
			}

			string strStylId = AdresarovaStruktura.Styl + intPoradi.ToString("000");

			Uri partUriObsah = PackUriHelper.CreatePartUri(new Uri(strVychoziSlozka + AdresarovaStruktura.ZnakoveStyly + "\\" + strStylId + "\\" + AdresarovaStruktura.Obsah + "\\" + strObsah, UriKind.Relative));

			Uri partUriStyl = PackUriHelper.CreatePartUri(new Uri(strVychoziSlozka + AdresarovaStruktura.ZnakoveStyly + "\\" + strStylId + ".xml", UriKind.Relative));
			...
			PackageRelationship rel = pkgpVychoziCastBalicku.CreateRelationship(pkgpStyl.Uri, TargetMode.Internal, Relace.ZnakovyStyl);
			...
			rel = UlozitJevyDoBalickuJakoObsah(fusFormat, strPripona, strJevyTmp, strVychoziSlozka, AdresarovaStruktura.ZnakoveStyly, strStylId, package, pkgpStyl, jvDigramy, "digramy", Relace.Digramy);
			mtdMetadata.Digramy.RelaceId = rel.Id;

			rel = UlozitJevyDoBalickuJakoObsah(fusFormat, strPripona, strJevyTmp, strVychoziSlozka, AdresarovaStruktura.ZnakoveStyly, strStylId, package, pkgpStyl, jvTrigramy, "trigramy", Relace.Trigramy);
			mtdMetadata.Trigramy.RelaceId = rel.Id;



			rel = UlozitJevyDoBalickuJakoObsah(fusFormat, strPripona, strJevyTmp, strVychoziSlozka, AdresarovaStruktura.ZnakoveStyly, strStylId, package, pkgpStyl, jvSlova, "slova", Relace.Slova);
			mtdMetadata.Slova.RelaceId = rel.Id;

			rel = UlozitJevyDoBalickuJakoObsah(fusFormat, strPripona, strJevyTmp, strVychoziSlozka, AdresarovaStruktura.ZnakoveStyly, strStylId, package, pkgpStyl, jvZnaky, "znaky", Relace.Znaky);
		...
		private static void UlozitStatistikuDoBalickuJakoObsah(IEnumerable<MetadataOJevu> glsOdstavcoveStyly, IEnumerable<MetadataOJevu> glsZnakoveStyly,
			...
			foreach (MetadataOJevu mtd in glsOdstavcoveStyly) {
				XmlNode xns = MetadataOJevu.VytvoritTagZMetadat(xd, mtd.MetadataBalicku, "styl", mtd.Popis, mtd.PocetUseku);
			...
			PackageRelationship rel = pkgpStatistiky.Package.CreateRelationship(pkgpStatistiky.Uri, TargetMode.Internal, Relace.Statistika);
		}
		...
		private static void UlozitStatistikyDoBalickuJakoPrehled(IEnumerable<MetadataOJevu> glsOdstavcoveStyly, IEnumerable<MetadataOJevu> glsZnakoveStyly,
			PackagePart pkgpStatistiky, VlastnostiBalicku vlbVlastnosti) {
			XmlDocument xd = new XmlDocument();
		...
		private static void UlozitStylyDoBalickuJakoPrehled(IEnumerable<MetadataOJevu> glsStyly, XmlNode xns, string strNazev) {
		...
		private static void UlozitOdstavcovyStylDoBalickuJakoObsah(MetadataOJevu mtdMetadata, List<MetadataOJevu> glsZnakoveStyly, PackagePart pkgpStyl) {
		...
		private static PackageRelationship UlozitSouborDoBalickuJakoObsah(FormatUlozeniSeznamu fusFormat, string strPripona,
			string tmpSlozka, string strVychoziSlozka, string strStylId, Package package, PackagePart pkgpStyl, string strTypJevu,
			string strTypRelace)
		{
		...
		private static PackageRelationship UlozitJevyDoBalickuJakoObsah(FormatUlozeniSeznamu fusFormat, string strPripona,
			...
			Sprava.UlozDoSouboru(jvJevy, strSlovaTmp, fusFormat == FormatUlozeniSeznamu.Xml ? FormatUlozeni.Xml : FormatUlozeni.Text);

			Uri partUriSlova = PackUriHelper.CreatePartUri(new Uri(strVychoziSlozka + strTypStylu + "\\" + strStylId + "\\" + AdresarovaStruktura.Obsah + "\\" + strSlova, UriKind.Relative));
			PackagePart pkgpSlova = package.CreatePart(partUriSlova,
						fusFormat == FormatUlozeniSeznamu.Xml ? MediaTypeNames.Text.Xml : MediaTypeNames.Text.Plain, CompressionOption.Maximum);
			using (FileStream fileStream = new FileStream(strSlovaTmp, FileMode.Open, FileAccess.Read)) {
				CopyStream(fileStream, pkgpSlova.GetStream());
		...
		public void UlozStatistiky() {
			...
			using (Package package = Package.Open(VystupniSoubor, FileMode.Create)) {
				...
				foreach (Jevy kvp in SkupinaJevu) {
					...
					jvsSprava.UlozDoSouboru(kvp, strSouborJevyXml);
					...
					jvsSprava.UlozDoSouboru(jvSlova, strSouborSlovaXml);
					jvsSprava.UlozDoSouboru(jvZnaky, strSouborZnakyXml);

					Uri partUriJevy = PackUriHelper.CreatePartUri(new Uri("Obsah\\" + strJevyXml, UriKind.Relative));
					PackagePart packagePartDocument = package.CreatePart(partUriJevy, MediaTypeNames.Text.Xml, CompressionOption.Maximum);
				...
				if (SloucitDetaily) {
					if (skjSlova != null)
						foreach (Jevy jvs in skjSlova) {
							...
							string strSouborSlovaXml = strTemp + strSlovaXml;
							jvsSprava.UlozDoSouboru(jvs, strSouborSlovaXml);
							UlozitSlovaNeboZnaky(package, strSlovaXml, strSouborSlovaXml, CsTypRelaceSlova);
						}
					if (skjZnaky != null)
						foreach (Jevy jvs in skjZnaky) {
							...
							string strSouborZnakyXml = strTemp + strZnakyXml;
							jvsSprava.UlozDoSouboru(jvs, strSouborZnakyXml);
							UlozitSlovaNeboZnaky(package, strZnakyXml, strSouborZnakyXml, CsTypRelaceZnaky);
						}
				}
			}
		...
		private static void UlozitSlovaNeboZnaky(Package package, string strOpcNazevSouboru, string strTempSouborJevuXml, string csTypRelace) {
			Uri partUriZnaky = PackUriHelper.CreatePartUri(new Uri("Obsah\\" + strOpcNazevSouboru, UriKind.Relative));
			PackagePart packagePartZnaky = package.CreatePart(partUriZnaky, MediaTypeNames.Text.Xml, CompressionOption.Maximum);
		...
		private static void ExtractPart(PackageRelationship docPackageRelationship, Package wdPackage, string outputDirectory, string outputFilePath)
		{
