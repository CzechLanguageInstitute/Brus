...
namespace Daliboris.Statistiky {
	...
	public class Jevy : ICollection<Jev>, IDictionary<string, Jev>, IJevyZdroje {
		...
		private readonly RazeniJevu _mrjRazeniJevu = new RazeniJevu();
		private readonly SortedDictionary<string, Jev> _mgdJevy = new SortedDictionary<string, Jev>(new RazeniJevu());
		...
		public CompareOptions ZpusobRazeni {
			get { return _mrjRazeniJevu.ZpusobRazeni; }
		}

		public string ID {
			get { return GetID(mstrJazyk, Identifikator); }
		}
		...
		public int Pocet { get { return _mgdJevy.Count; } }
		...
		public TypJevu Druh { get; set; } = TypJevu.Ostatni;
		public Zdroj Zdroj { get; set; } = new Zdroj();
		...
		public Jevy(RazeniJevu rjRazeni) {
			_mgdJevy = new SortedDictionary<string, Jev>(rjRazeni);
		}

		public Jevy(TypJevu tpDruh) {
			Druh = tpDruh;
		}
		public Jevy(TypJevu tpDruh, Zdroj zdZdroj) {
			Druh = tpDruh;
			Zdroj = zdZdroj;
		}
		...
		public Jevy(TypJevu tpDruh, string strCestaKeZdroji)
			: this(strCestaKeZdroji) {
			Druh = tpDruh;
		}
		public Jevy(string strCestaKeZdroji) {
			Zdroj = new Zdroj(strCestaKeZdroji);
		}
		...
		public Jevy(TypJevu tpDruh, string strCestaKeZdroji, string strJazyk, string strIdentifikator)
			: this(strCestaKeZdroji, strJazyk, strIdentifikator) {
			Druh = tpDruh;
		}

		public Jevy(TypJevu tpDruh, string strCestaKeZdroji, string strJazyk, string strIdentifikator, string popis)
			: this(tpDruh, strCestaKeZdroji, strJazyk, strIdentifikator)
		{
			Popis = popis;
		}
		...
		public Jev this[string identifikator] {
			get { return _mgdJevy.ContainsKey(identifikator) ? _mgdJevy[identifikator] : null; }
		}
		...
		public Jev Append(string strJazyk, string strNazev, object objObsah, string strRetrograd, int intPocet) {
			...
			Add(jv);
			jv.Pocet = _mgdJevy[strNazev].Pocet;
		...
		public void Add(Jev item) {
			if (_mgdJevy.ContainsKey(item.Nazev)) {
				_mgdJevy[item.Nazev].Pocet += item.Pocet;
				_mgdJevy[item.Nazev].Kontexty.AddRange(item.Kontexty);
			}
			else {
				_mgdJevy.Add(item.Nazev, item);
			}
		}
		...
		public bool Contains(Jev item) {
			return _mgdJevy.ContainsKey(item.Nazev);
		}

		public void CopyTo(Jev[] array, int arrayIndex) {
			_mgdJevy.Values.CopyTo(array, arrayIndex);
		}


		public int Count {
			get { return _mgdJevy.Count; }
		}
		...
		public bool Remove(Jev item) {
			return _mgdJevy.Remove(item.Nazev);
		}
		...
		public IEnumerator<Jev> GetEnumerator() {
			return _mgdJevy.Values.GetEnumerator();
		}
...
*/
		...
		public void Add(KeyValuePair<string, Jev> item) {
			_mgdJevy.Add(item.Key, item.Value);
		}

		public bool Contains(KeyValuePair<string, Jev> item) {
			return _mgdJevy.ContainsKey(item.Key);
		}
		...
		public bool Remove(KeyValuePair<string, Jev> item) {
			return _mgdJevy.Remove(item.Key);
		}
