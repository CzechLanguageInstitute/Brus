...
namespace Daliboris.Statistiky.Word {
	...
	public class DocxRotor  {
		...
		private int poradiOdstavce;
		private string _souborDocx;
	    private string _aktualniLokace;
		private DocxReader _reader;
		//Dictionary<string, Jevy> mgdcJevy;
		private DocxRotorNastaveni _nastaveni = new DocxRotorNastaveni();

		//public Dictionary<string, Jevy> SeznamJevu {
		//   get { return mgdcJevy; }
		//}
		public SkupinaJevu SkupinaJevu { get; private set; }
		...
		public DocxRotorNastaveni Nastaveni {
		...
		public DocxRotor(DocxRotorNastaveni nstNastaveni) {
			Nastaveni = nstNastaveni;
		}

		public DocxRotor(string strSouborDocx, DocxRotorNastaveni nstNastaveni) {
			SouborDocx = strSouborDocx;
			Nastaveni = nstNastaveni;
		}
		public DocxRotor(string strSouborDocx) {
			SouborDocx = strSouborDocx;
		}
		...
		private void mdxrReader_ZacatekOdstavce(object sender, ParagraphEventArgs ev) {
			...
			if (_textOdstavceSb != null)
				_nejdelsiOdstavec = _textOdstavceSb.Length > _nejdelsiOdstavec ? _textOdstavceSb.Length : _nejdelsiOdstavec;
			_textOdstavce = null;
        ...
        private void mdxrReader_KonecOdstavce(object sender, ParagraphEventArgs ev)
        {
			...
			string sText = _nastaveni.OdstranitPocatecniAKoncoveMezery ? _textOdstavceSb.ToString().Trim() : _textOdstavceSb.ToString();
			...
			XElement root = new XElement("p", 
				...
				new XAttribute("i", ++poradiOdstavce),
			...
			{
				Jevy tmpJevy = GetJevy(jevy.Jazyk, jevy.Identifikator, jevy.Popis, jevy.Typ);
	    ...
	    private void ZpracovatZnakovyStylJakoJev(TextRunEventArgs ev)
	    {
	        ...
	        if (_nastaveni.ZahrnoutTextOdstavce)
	        {
	            ...
	            if (!SkupinaJevu.ContainsID((sOdstavecZnakID)))
	            {
	            ...
	            SkupinaJevu[sOdstavecZnakID].Add(jv.Clone());
	        }
	    }
		...
		private Jevy GetJevy(string strLanguage, string strStyleID, string strStyleUIName, TypJevu tpTyp) {
			...
			return SkupinaJevu[sID];
		}

		private static Jevy GetJevy(SkupinaJevu skupina, string souborDocx,
			string strLanguage, string styleId, string styleUiName, TypJevu tpTyp)
		{
			string sId = Jevy.GetID(strLanguage, styleId);
			if (!skupina.ContainsID(sId))
			{
				Jevy jvs = new Jevy(tpTyp, souborDocx, strLanguage, styleId, styleUiName);
				skupina.Add(jvs);
			}
			return skupina[sId];
		}

		public void ZpracujDocx() {
